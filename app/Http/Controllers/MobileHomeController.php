    ...
    public function classList(Request $request)
    {
        ...
        foreach ($classes as $data) {
            ...
            if ($data->course_cover_image) {
                ...
                if (str_contains($data->course_cover_image, "course-s3") || str_contains($data->course_cover_image, "lesson-s3")) {
                    ...
                    $data->course_cover_image = $presignedUrl; // Menimpa key asli
                    $data->full_img_path = $presignedUrl;      // Menimpa key full_img_path
                    
                } else {
                    // Jika file lokal/public storage
                    $localPath = url("/") . Storage::url('public/class/cover/') . $data->course_cover_image;
                    $data->course_cover_image = $localPath;
                    $data->full_img_path = $localPath;
                }
            } else {
