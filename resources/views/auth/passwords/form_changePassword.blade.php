...
@section('body')
                ...
                <div class="col-md-8">
                    <div class="card" style="border-radius: 30px;">
                        ...
                        <div class="card-body d-flex justify-content-center align-items-center mb-5">
                            <form method="POST" action="{{ route('password.updateChange') }}" id="reset-password-form" class="w-100">
                                @csrf
                                <div class="mb-3">
                                    <label for="password" class="form-label">Last Password</label>
                                    <div class="input-group">
                                        <input type="password" class="password form-control @error('password') is-invalid @enderror" id="input_lastPass" name="lastPass" required autocomplete="new-password" minlength="8">
                                        <button class="btn btn-outline-secondary" type="button" id="toggleLastPass">
                                            <i class="fa fa-eye"></i>
                                        </button>
                                    </div>
                                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('toggleLastPass').addEventListener('click', function () {
                var passwordInput = document.getElementById('input_lastPass');
                var type = passwordInput.getAttribute('type') === 'password' ? 'text' : 'password';
                passwordInput.setAttribute('type', type);
        
                var eyeIcon = document.querySelector('#toggleLastPass i');
