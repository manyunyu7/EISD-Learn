                        ...
                        <div class="panel panel-default" style="margin-bottom: 20px;">
                            <div class="panel-body">
                                ...
                                @if (!empty($question->image))
                                    <img src="{{ Storage::url('public/exam/question')."/".$question->image }}" alt="Question Image" style="max-width: 100%;">
                                @endif

                                @if (stripos($question->question_type, 'single') !== false)
                                    <div class="radio">
                                        @foreach(json_decode($question->choices, true) as $choice)
                                            <div>
                                                <label>
                                                    <input type="radio" name="answers[{{ $question->id }}]" value="{{ $choice['text'] }}"> {{ $choice['text'] }}
                                                </label>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
                                    <div class="checkbox">
                                        @foreach(json_decode($question->choices, true) as $choice)
                                            <div>
                                                <label>
                                                    <input type="checkbox" name="answers[{{ $question->id }}][]" value="{{ $choice['text'] }}"> {{ $choice['text'] }}
                                                </label>
                                            </div>
                                        @endforeach
                                    </div>
